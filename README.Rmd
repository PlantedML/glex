...
For an XGBoost binary model, the prediction is built on a **raw margin** (log-odds) scale:

$$
F(x) = b + \sum_{t=1}^{T} f_t(x), \qquad p(x) = \sigma(F(x)) = \frac{1}{1 + e^{-F(x)}}.
$$

With `objective = "binary:logistic"`, XGBoost optimizes logistic loss in terms of this same margin:
...
- A positive component (in `m` or `shap`) increases log-odds and therefore increases probability.
