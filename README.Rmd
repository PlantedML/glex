...
[![R-universe version](https://PlantedML.r-universe.dev/glex/badges/version)](https://PlantedML.r-universe.dev/glex)
[![CRAN status](https://img.shields.io/cran/v/glex)](https://CRAN.R-project.org/package=glex)
<!-- badges: end -->
...
> [arXiv](https://arxiv.org/abs/2410.13448) • [ICML 2025 Proceedings](https://proceedings.mlr.press/v267/liu25bm.html)
...
The examples below use [`xgboost`][xgb] as the general example; `ranger` and
[`randomPlantedForest`][rpf] models are also supported and work analogously,
as described in the [Supported Models](#supported-models) section above.
...
Note that `xgboost` requires `matrix` input and does not support categorical predictors.
...
Using the model object and a dataset to explain (such as a test set in this case), we can create a `glex` object.
