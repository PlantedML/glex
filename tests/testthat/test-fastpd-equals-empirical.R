...
test_that("FastPD equals empirical leaf weighting for lower interactions", {
  ...
  sorted_cols_fastpd <- sapply(colnames(fastpd$m), function(col) {
    paste(sort(strsplit(col, ":")[[1]]), collapse = ":")
  })
  colnames(fastpd$m) <- sorted_cols_fastpd

  sorted_cols_empirical <- sapply(
    colnames(empirical_leaf_weighting$m),
    function(col) paste(sort(strsplit(col, ":")[[1]]), collapse = ":")
  )
