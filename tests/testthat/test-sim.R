...
test_that("binary: Prediction is approx. same as sum of shap + intercept, training data", {
  expect_equal(
    resbin_train$intercept + rowSums(resbin_train$shap),
    predbin_train,
    tolerance = 1e-5
  )
})
...
test_that("binary: Prediction is approx. same as sum of shap + intercept, test data", {
  expect_equal(
    resbin_test$intercept + rowSums(resbin_test$shap),
    predbin_test,
    tolerance = 1e-5
  )
})
...
test_that("classif: Prediction is approx. same as sum of decomposition + intercept, training data", {
  expect_equal(
    resbin_train$intercept + rowSums(resbin_train$m),
    predbin_train,
    tolerance = 1e-5
  )
})
...
test_that("binary: Prediction is approx. same as sum of decomposition + intercept, test data", {
  expect_equal(
    resbin_test$intercept + rowSums(resbin_test$m),
    predbin_test,
    tolerance = 1e-5
  )
})
