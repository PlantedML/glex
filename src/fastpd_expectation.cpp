...
double augmentExpectation(NumericVector &x, NumericMatrix &tree, NumericVector &to_explain, LeafData &leaf_data)
{
  ...
  while (!to_process.empty())
  {
    ...
    {
      if (ComparisonPolicy::compare(x[current_feature], split))
      {
...
{
  return augmentExpectation<glex::WeakComparison>(x, tree, to_explain, leaf_data);
}

double augmentExpectationStrictComparison(NumericVector &x, NumericMatrix &tree, NumericVector &to_explain, LeafData &leaf_data)
{
  return augmentExpectation<glex::StrictComparison>(x, tree, to_explain, leaf_data);
}
