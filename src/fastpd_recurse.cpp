...
NumericMatrix recurseMarginalizeS_(
    NumericMatrix &x, NumericMatrix &tree,
    ...
    LeafData &leaf_data)
{
  ...
  else
  {
    ...
    // Call both children, they give a matrix each of all obs and subsets
    NumericMatrix mat_yes = recurseMarginalizeS_(x, tree, Ss, yes, leaf_data);
    NumericMatrix mat_no = recurseMarginalizeS_(x, tree, Ss, no, leaf_data);
